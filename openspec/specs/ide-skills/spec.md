...
Windsurf/Devin IDE skills в `.windsurf/skills/` автоматизируют workflow бенчмарка.
Пользователь не запускает python-скрипты напрямую — всё через skills
`@benchmark-01-gen-task` → `@benchmark-05-aggregate`.
...
- **AND** готовит промпт (читает задачи, анонимизированные пргоны)
- **AND** показывает промпт для копипаста
- **WHEN** пользователь говорит "готово"
