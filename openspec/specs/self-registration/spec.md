...
Саморегистрация моделей в `models.yaml` и `judges.yaml`. Модели регистрируют
себя автоматически при старте роли через `register_model.py --auto` и
`assign_judges.py --add-judge`. Пользователь не редактирует yaml-файлы руками.
