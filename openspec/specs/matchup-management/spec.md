...
NNN SHALL быть порядковым номером вердикта внутри директории задачи/корзины,
начиная с 001, формат — zero-padded 3 цифры (`001.json`). Сервер вычисляет номер
как `len(существующих файлов) + 1`.
...
- **WHEN** из середины директории удалён файл (например остался только `002.json`)
- **THEN** следующий вердикт перезапишет существующий (`len + 1` вместо `max + 1`)
- **AND** это известный дефект `tools/server.py:record_verdict`, исправление — отдельной задачей
...
ELO-движок SHALL упорядочивать вердикты по паре (`date`, `_matchup_id`):
сначала дата из поля `date`, затем идентификатор `<task-id>/<NNN>`.
Два вердикта в один день упорядочиваются по NNN.
